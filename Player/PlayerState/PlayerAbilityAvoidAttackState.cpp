...
void PlayerAbilityAvoidAttackState::Enter() {
	m_Owner->ChangeMotion(MotionAbilityAvoidAttack, false);
	m_Enemy = m_Owner->FindTargetEnemy();
	m_Owner->ResetVelocity();
	m_Owner->SetNextAttackStateFrag(false);
}

void PlayerAbilityAvoidAttackState::Execute(float delta_time) {
...
void PlayerAbilityAvoidAttackState::Exit() {
	m_Owner->SetNextAttackStateFrag(false);
	m_Owner->ResetVelocity();
}

void PlayerAbilityAvoidAttackState::OnMessage(const std::string& message, void* param) {}
