...
void PlayerAbilityPushAttackState::Enter() {
	m_Owner->ChangeMotion(MotionAbilityPushAttack, false);
	m_Owner->ResetVelocity();
	m_Owner->SetNextAttackStateFrag(false);
	m_Enemy = m_Owner->FindTargetEnemy();
}

void PlayerAbilityPushAttackState::Execute(float delta_time) {
...
void PlayerAbilityPushAttackState::Exit() {
	m_Owner->ResetVelocity();
	m_Owner->SetNextAttackStateFrag(false);
}

void PlayerAbilityPushAttackState::OnMessage(const std::string& message, void* param) {}

bool PlayerAbilityPushAttackState::Distance()const {
