...
void PlayerAbilitySlamAttackState::Enter() {
	m_Owner->ChangeMotion(MotionAbilitySlamAttack, false);
	m_Owner->ResetVelocity();
	m_Owner->SetNextAttackStateFrag(false);
	m_Enemy = m_Owner->FindTargetEnemy();
}

void PlayerAbilitySlamAttackState::Execute(float delta_time) {
...
void PlayerAbilitySlamAttackState::Exit() {
	m_Owner->ResetVelocity();
	m_Owner->SetNextAttackStateFrag(false);
}

void PlayerAbilitySlamAttackState::OnMessage(const std::string& message, void* param) {}