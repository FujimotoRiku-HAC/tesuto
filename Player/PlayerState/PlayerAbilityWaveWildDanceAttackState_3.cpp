...
void PlayerAbilityWaveWildDanceAttackState_3::Enter() {
	m_Owner->ChangeMotion(MotionAbilityWaveWildDance_3, false);
	m_Owner->ResetVelocity();
	m_Owner->SetNextAttackStateFrag(false);
	m_Enemy = m_Owner->FindTargetEnemy();
}

void PlayerAbilityWaveWildDanceAttackState_3::Execute(float delta_time) {
...
void PlayerAbilityWaveWildDanceAttackState_3::Exit() {
	m_Owner->ResetVelocity();
	m_Owner->SetNextAttackStateFrag(false);
}

void PlayerAbilityWaveWildDanceAttackState_3::OnMessage(const std::string& message, void* param) {}