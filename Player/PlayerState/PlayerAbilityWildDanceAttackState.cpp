...
void PlayerAbilityWildDanceAttackState::Enter() {
	m_Owner->ChangeMotion(MotionAbilityWildDanceAttack, false);
	m_Owner->ResetVelocity();
	m_Owner->SetNextAttackStateFrag(false);
	m_Enemy = m_Owner->FindTargetEnemy();
}

void PlayerAbilityWildDanceAttackState::Execute(float delta_time) {
...
void PlayerAbilityWildDanceAttackState::Exit() {
	m_Owner->ResetVelocity();
	m_Owner->SetNextAttackStateFrag(false);
}

void PlayerAbilityWildDanceAttackState::OnMessage(const std::string& message, void* param) {}